...
def upload_files():
    ...
    for file in files:
        if file and file.filename:
            ...
            # Delay formula: 200ms base + 100ms per GB, max 3 seconds
            delay = min(2.0, 0.3 + (file_size_gb * 0.1))
            time.sleep(delay)
...
def analyze_logs():
    try:
        ...
        print(f"Received case number: {case_number}")  # Debug print
        
        script_path = os.path.join('backend', 'log.py')
        print(f"Script path: {script_path}")  # Debug print

        process = subprocess.Popen(
            [python_executable, script_path, case_number],
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE
        )
        
        stdout, stderr = process.communicate()
        print(f"Process output: {stdout.decode('utf-8')}")  # Debug print
        print(f"Process errors: {stderr.decode('utf-8')}")  # Debug print
...
def analyze_network():
    try:
        ...
        script_path = os.path.join('backend', 'network.py')
        print(f"Script path: {script_path}")  # Debug print
        
        python_executable = sys.executable
        print(f"Using Python from: {python_executable}")  # Debug print
...
def analyze_file():
    try:
        ...
        script_path = os.path.join('backend', 'file.py')
        print(f"Script path: {script_path}")  # Debug print
